...
void
MmWavePointToPointEpcHelper::AddSenb (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint16_t cellId) //sychoi 170520
{
  NS_LOG_UNCOND("ERROR: AddSenb run in mmWave simulation!");
}
...
uint8_t
MmWavePointToPointEpcHelper::ActivateEpsBearerDc (Ptr<NetDevice> ueDevice, uint64_t imsi, Ptr<EpcTft> tft, EpsBearer bearer, uint8_t dcType) //sychoi 170520
{
  NS_LOG_UNCOND("ERROR: ActivateEpsBearerDc run in mmWave simulation!");
  return -1;
}
...
Ptr<EpcMme>
MmWavePointToPointEpcHelper::GetMme () //sychoi 170520
{
  return m_mme;
}
